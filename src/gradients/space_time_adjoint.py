...
each and nothing may grow faster than they do:
  ...
  * The polarizability is the one genuinely (M x M) object, Pi = G^o * G^v,
    ...
    gradient, so a change to the arithmetic reaches both.
  * Everything downstream lives in the auxiliary basis. proj(tau), (ntau, naux,
    naux), is the ONE object kept whole -- it is the result of the N^3 sweep,
    every frequency is a fixed linear combination of it -- and adjoints come
    back in the same space, as projbar(tau). Frequencies are visited in blocks
...
from src.SingleReference.LinearResponse.space_time import (  # noqa: F401
    chi0_imaginary_frequency, frequency_blocks, laplace_representation_error,
    owned_frequency_blocks, polarizability_projected_sweep,
    rpa_correlation_energy_space_time, split_branches, three_index_ov,
    three_index_slice, tile_rows)
...
def polarizability_tau(X, D, eps, nocc, grid, mu=None, tile_gb=ISDF_TILE_GB,
    ...
    caller assembling a partial sweep; the sum over disjoint subsets is the
    whole.
    """
...
def chi0_frequency(X, D, eps, nocc, grid, mu=None, tile_gb=ISDF_TILE_GB,
    ...
    """
    return chi0_imaginary_frequency(X, D, eps, nocc, grid, mu=mu,
...
def polarizability_backward(proj_bar, X, D, eps, nocc, grid, mu=None,
    ...
    over tau, so the sum over disjoint subsets is the whole.
    """
    X_o, X_v, e_o, e_v, _, occ, virt = split_branches(X, eps, nocc, mu)
    M, naux = X.shape[0], D.shape[1]
    ...
    for k in which:
        ...
        pb = 0.5 * (proj_bar[k] + proj_bar[k].T)
...
def chi0_backward(chi0_bar, X, D, eps, nocc, grid, mu=None,
    ...
    adjoint transposes them, projbar = cosft_wt^T chi0bar -- (ntau, naux, naux),
    with no (nfreq, naux, naux) copy on the way.
    """
    proj_bar = np.tensordot(grid.cosft_wt, chi0_bar, axes=(0, 0))
    return polarizability_backward(proj_bar, X, D, eps, nocc, grid, mu=mu,
                                   tile_gb=tile_gb)
...
def rpa_energy_and_adjoint(X, D, eps, nocc, grid, mu=None, want_grad=True,
    ...
    and `counter_term` are that routine's arguments and are documented there.
    want_grad=False is the forward alone.
    ...
    overwritten by its own adjoint and folded into projbar, and the axis is
    never whole. Each block is transformed a second time here, since the
    forward kept proj(tau) and not the blocks: one ntau x naux^2 tensordot per
    block against the naux^3 factorization the same block pays anyway.
    ...
    e_c, tape = rpa_correlation_energy_space_time(
        X, D, eps, nocc, grid, mu=mu, tile_gb=tile_gb, screening=screening,
        counter_term=counter_term, want_tape=True)
    ...
    for ks, blk in owned_frequency_blocks(proj_tau, grid.cosft_wt, tile_gb,
        ...
        proj_bar += np.tensordot(grid.cosft_wt[ks], blk, axes=(0, 0))
...
def rpa_frequency_traces(X, D, eps, nocc, grid, mat, mu=None,
    ...
    serves a whole scan over the solvent's single-pole energy.
    """
...
def screened_interaction_tau_backward(Wt_bar, proj_tau, grid, Ctw,
    ...
    for ks, chi0_blk in owned_frequency_blocks(proj_tau, grid.cosft_wt, tile_gb,
        ...
        proj_bar += np.tensordot(grid.cosft_wt[ks], chi0_blk, axes=(0, 0))
    return proj_bar
...
def selfenergy_block_backward(a_re, a_im, X, D, eps, nocc, grid, states,
    ...
    for s, p in enumerate(states):
        ...
        X_bar += Xs
        D_bar += Ds
    return eps_bar + eps2, X_bar, D_bar
